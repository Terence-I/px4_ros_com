...
class OffboardControl(Node):
    ...
    def __init__(self):
        ...
        qos_profile_sub = QoSProfile(reliability=ReliabilityPolicy.BEST_EFFORT, durability=DurabilityPolicy.VOLATILE, history=HistoryPolicy.KEEP_LAST, depth=3)
    ...
    def publish_trajectory_setpoint(self):
        ...
        if self.uav_pose1 is not None:
        	...
        	xg = self.current_pose.pose.pose.position.y
        	yg = self.current_pose.pose.pose.position.x
        	zg = -1 * self.current_pose.pose.pose.position.z
